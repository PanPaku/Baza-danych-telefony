...
SELECT telefony.marka, telefony.model
FROM klienci
JOIN zwroty ON klienci.klient_ID = zwroty.klient_ID
...
SELECT telefony.marka, telefony.model
FROM telefony
INNER JOIN gwarancje ON telefony.telefon_ID = gwarancje.telefon_ID
...
SELECT telefony.marka AS nazwa_telefonu, telefony.model AS model_telefonu, COUNT(*) AS liczba_zamowien
FROM telefony 
INNER JOIN zamówienia  ON telefony.telefon_ID = zamówienia.telefon_ID
WHERE YEAR(zamówienia.data_zam) = YEAR(CURRENT_DATE)
GROUP BY telefony.marka, telefony.model
HAVING COUNT(*) = 3;
